package com.example.rnmapp.location_feature.domain.use_cases.get_next_page
...
class GetNextLocationPageUseCase @Inject constructor(
    private val repository: LocationRepository
) {
    operator fun invoke(url: String): Flow<Resource<LocationResponse>> = flow {
        try {
            emit(Resource.Loading<LocationResponse>())
            val next = repository.getNextLocationPage(url)
            emit(Resource.Success<LocationResponse>(next))
